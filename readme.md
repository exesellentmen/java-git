...
**Промежуточные операции** - Возвращают тип Stream

**1.2 filter - метод для фильтрации данных**
Пример:<br>
...
**1.3 map - медот для преобразование одного массива в другой массив**
Получить все id элементов
```java
arrayList.stream().map(el -> el.id);
```

**1.4 flatmap - расширенный метод map для получение всех элементов из вложенных списков**
...
**1.10 collect - преобразует стрим в выбранный тип**
Получить из stream тип List
```java
arrayList.stream().collect(Collectors.toList());
```

**1.11 forEach - перебрать элементы, не возвращает результат, можно использовать для вывода сообщений или для суммы и т д**
...
**1.12 count - подсчет количество элементов**

**1.13 min - минимальный элемент**
```java
Optional<Product> product = arrayList.stream().min((e1, e2) -> e1.name.compareTo(e2.name));
```

**1.15 findFirst() - получить первый элемент для поиска**
...
**1.17 Получение результата из всех элементов**
